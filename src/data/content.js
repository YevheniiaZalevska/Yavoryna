import {
  Shield,
  Sparkles,
  HeartHandshake,
  Shirt,
  Layers,
  Leaf
} from "lucide-react";

export const CONTACTS = {
  telegram: {
    label: "Telegram",
    href: "https://t.me/OlZalevska", 
  },
  facebook: {
    label: "Facebook",
    href: "https://www.facebook.com/share/g/1RNGE1GYj9/", 
  },
    insta: {
    label: "Instagram",
    href: "https://www.instagram.com/olenazal?igsh=ZWV5MGljdThid2d6", 
  },
};

export const BRAND_NAME = "Яворина";
export const BRAND_SUBTITLE = "Волонтерська ініціатива";
export const REGION_LABEL = "Дніпропетровська область";

export const NAV = [
  {
    id: "products",
    label: "Наші вироби",
    children: [
      { id: "nets", label: "Маскувальні сітки" },
      { id: "chugaistryky", label: "Чугайстрики" },
      { id: "other", label: "Інше" },
    ],
  },
  { id: "gallery", label: "Звіти" },
  { id: "awards", label: "Наші нагороди" },
];

export const SERVICES = [
  {
    id: "nets",
    title: "Маскувальні сітки різних модифікацій",
    desc: "Плетемо сітки під сезон, місцевість та призначення (на фортифікаційні споруди або техніку). Узгоджуємо розмір.",
    bullets: ["Кольори під сезон", "Розміри під запит", "Фотозвіт у процесі"],
    icon: Shield,
  },
  {
    id: "chugaistryky",
    title: "Чугайстрики – індивідуальні маскувальні костюми",
    desc: "Робимо чугайстрики та маскувальні елементи під конкретні задачі.",
    bullets: ["Під задачу", "Матеріали за наявністю", "Уточнюємо терміни"],
    icon: Leaf,
  },
  {
    id: "underwear",
    title: "Білизна",
    desc: "Виготовляємо білизну та адаптовану білизну за потребами.",
    bullets: ["Під актуальні потреби", "Пакування наборами", "Відправка"],
    icon: Shirt,
  },
  {
    id: "mats",
    title: "Килимки (утеплення/побут)",
    desc: "Робимо килимки для побутових потреб.",
    bullets: ["Різні формати", "Можна партіями", "Уточнюємо наявність"],
    icon: Layers,
  },
  {
    id: "socks",
    title: "Теплі шкарпетки",
    desc: "В’язання для фронту та шпиталів.",
    bullets: ["Тепло і надійно", "Набори/партії", "Відправка"],
    icon: HeartHandshake,
  },

  { id: "awards", label: "Нагороди" },
];
''
export const BASE = import.meta.env.BASE_URL;

export const GALLERY = [
  { src: `${BASE}gallery/other/02.jpg`, alt: "Волонтери за роботою", tag: "Команда", group: "other" },
  { src: `${BASE}gallery/other/03.jpg`, alt: "Процес", tag: "Процес", group: "other" },
  { src: `${BASE}gallery/chugaistryky/04.jpg`, alt: "Чугайстрики", tag: "Чугайстрики", group: "chugaistryky" },
  { src: `${BASE}gallery/other/05.jpg`, alt: "Волонтери за роботою", tag: "Команда", group: "other" },
  { src: `${BASE}gallery/other/06.jpg`, alt: "Звіт", tag: "Звіт", group: "other" },
  { src: `${BASE}gallery/other/07.jpg`, alt: "Готові вироби", tag: "Посилка", group: "other" },
  { src: `${BASE}gallery/other/08.jpg`, alt: "Робоче місце / матеріали", tag: "Процес", group: "other" },
  { src: `${BASE}gallery/other/09.jpg`, alt: "Відправка", tag: "Відправка", group: "other" },
  { src: `${BASE}gallery/other/10.jpg`, alt: "Готові вироби", tag: "Фідбек", group: "other" },
  { src: `${BASE}gallery/other/11.jpg`, alt: "Плетіння сітки", tag: "Готові вироби", group: "other" },
  { src: `${BASE}gallery/other/12.jpg`, alt: "Волонтери за роботою", tag: "Волонтери за роботою", group: "other" },
  { src: `${BASE}gallery/other/81.jpg`, alt: "Ракета", tag: "Ракета", group: "other" },
  { src: `${BASE}gallery/other/84.jpg`, alt: "Волонтери Яворина", tag: "Волонтери Яворина", group: "other" },
  { src: `${BASE}gallery/warm/13.jpg`, alt: "Шапка", tag: "Шапка", group: "warm" },
  { src: `${BASE}gallery/warm/14.jpg`, alt: "Подушки", tag: "Подушки", group: "warm" },
  { src: `${BASE}gallery/warm/15.jpg`, alt: "Шкарпетки", tag: "Шкарпетки", group: "warm" },
  { src: `${BASE}gallery/warm/16.jpg`, alt: "Шапка", tag: "Шапка", group: "warm" },
  { src: `${BASE}gallery/warm/17.jpg`, alt: "Килимки", tag: "Килимки", group: "warm" },
  { src: `${BASE}gallery/warm/27.jpg`, alt: "Шкарпетки", tag: "Шкарпетки", group: "warm" },
  { src: `${BASE}gallery/warm/28.jpg`, alt: "Допомога війську", tag: "Допомога війську", group: "warm" },
  { src: `${BASE}gallery/nets/01.jpg`, alt: "Готова маскувальна сітка", tag: "Відправка", group: "nets" },
  { src: `${BASE}gallery/nets/19.jpg`, alt: "Сітки", tag: "Літня сітка", group: "nets" },
  { src: `${BASE}gallery/nets/20.jpg`, alt: "Сітки", tag: "Сітки", group: "nets" },
  { src: `${BASE}gallery/nets/21.jpg`, alt: "Сітки", tag: "Сітки в природі", group: "nets" },
  { src: `${BASE}gallery/nets/22.jpg`, alt: "Сітки", tag: "Сітки", group: "nets" },
  { src: `${BASE}gallery/nets/23.jpg`, alt: "Сітки", tag: "Зимова сітка", group: "nets" },
  { src: `${BASE}gallery/nets/24.jpg`, alt: "Сітки", tag: "Зимова сітка", group: "nets" },
  { src: `${BASE}gallery/nets/25.jpg`, alt: "Сітки", tag: "Готова сітка", group: "nets" },
  { src: `${BASE}gallery/nets/18.jpg`, alt: "Сітки", tag: "Готова сітка", group: "nets" },
  { src: `${BASE}gallery/nets/26.jpg`, alt: "Сітки", tag: "Сітка в реальному житті", group: "nets" },
  { src: `${BASE}gallery/nets/81.jpg`, alt: "Сітки", tag: "Готовий виріб", group: "nets" },
  { src: `${BASE}gallery/nets/90.jpg`, alt: "Маскувальні сітки", tag: "Маскувальні сітки", group: "nets" },
  { src: `${BASE}gallery/nets/91.jpg`, alt: "Маскувальні сітки", tag: "Маскувальні сітки", group: "nets" },
  { src: `${BASE}gallery/chugaistryky/33.jpg`, alt: "Чугайстрики", tag: "Зимові чугайстрики", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/34.jpg`, alt: "Чугайстрики", tag: "Чугайстрики на природі", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/35.jpg`, alt: "Чугайстрики", tag: "Чугайстрики", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/36.jpg`, alt: "Чугайстрики", tag: "Готовий виріб", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/37.jpg`, alt: "Чугайстрики", tag: "Чугайстрики звіт", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/38.jpg`, alt: "Чугайстрики", tag: "Чугайстрики", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/43.jpg`, alt: "Чугайстрики", tag: "Готові угайстрики", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/44.jpg`, alt: "Чугайстрики", tag: "Чугайстрики", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/45.jpg`, alt: "Чугайстрики", tag: "Чугайстрики на військових", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/46.jpg`, alt: "Чугайстрики", tag: "Чугайстрики взимку", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/80.jpg`, alt: "Чугайстрики", tag: "Чугайстрики в полі", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/47.jpg`, alt: "Чугайстрики", tag: "Чугайстрики в полі", group: "chugaistryky" },
  { src: `${BASE}gallery/chugaistryky/48.jpg`, alt: "Чугайстрики", tag: "Чугайстрики в полі", group: "chugaistryky" },
  { src: `${BASE}gallery/other/28.jpg`, alt: "Команда", tag: "Команда", group: "other" },
  { src: `${BASE}gallery/other/29.jpg`, alt: "Звіт", tag: "Звіт", group: "other" },
  { src: `${BASE}gallery/other/30.jpg`, alt: "Відгук", tag: "Відгук", group: "other" },
  { src: `${BASE}gallery/other/31.jpg`, alt: "Готовий виріб", tag: "Готовий виріб", group: "other" },
  { src: `${BASE}gallery/other/32.jpg`, alt: "Фідбек", tag: "Фідбек", group: "other" },
  { src: `${BASE}gallery/other/47.jpg`, alt: "Фідбек", tag: "Фідбек", group: "other" },
  { src: `${BASE}gallery/other/48.jpg`, alt: "Команда", tag: "команда", group: "other" },
  { src: `${BASE}gallery/other/49.jpg`, alt: "Відгук", tag: "Відгук", group: "other" },
  { src: `${BASE}gallery/other/50.jpg`, alt: "Військові з відгуком", tag: "Військові з відгуком", group: "other" },
  { src: `${BASE}gallery/other/51.jpg`, alt: "Відправка", tag: "Відправка", group: "other" },
  { src: `${BASE}gallery/other/52.jpg`, alt: "Фідбек", tag: "Фідбек", group: "other" },
  { src: `${BASE}gallery/other/53.jpg`, alt: "Звіт", tag: "Звіт", group: "other" },
  { src: `${BASE}gallery/other/54.jpg`, alt: "Звіт", tag: "Звіт", group: "other" },
  { src: `${BASE}gallery/other/55.jpg`, alt: "Фідбек", tag: "Фідбек", group: "other" },
  { src: `${BASE}gallery/other/56.jpg`, alt: "Готові вироби", tag: "Готові вироби", group: "other" },
  { src: `${BASE}gallery/other/57.jpg`, alt: "Волонтери", tag: "Волонтери", group: "other" },
  { src: `${BASE}gallery/other/58.jpg`, alt: "Сітка", tag: "Готова сітка", group: "other" },
  { src: `${BASE}gallery/other/59.jpg`, alt: "Посилка", tag: "Посилка", group: "other" },
  { src: `${BASE}gallery/other/60.jpg`, alt: "Відправка", tag: "Відправка", group: "other" },
  { src: `${BASE}gallery/other/61.jpg`, alt: "Фідбек", tag: "Фідбек", group: "other" },
  { src: `${BASE}gallery/other/62.jpg`, alt: "Волонтери в роботі", tag: "Волонтери в роботі", group: "other" },
  { src: `${BASE}gallery/other/63.jpg`, alt: "Робоче місце", tag: "Робоче місце", group: "other" },
  { src: `${BASE}gallery/other/64.jpg`, alt: "Відгук", tag: "Відгук", group: "other" },
  { src: `${BASE}gallery/other/82.jpg`, alt: "Волонтери Камянського", tag: "Волонтери Камянського", group: "other" },
  { src: `${BASE}gallery/other/83.jpg`, alt: "Маскувальна сітка", tag: "Маскувальна сітка", group: "other" },
  { src: `${BASE}gallery/awards/66.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/67.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/68.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/69.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/70.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/71.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/78.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/73.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/74.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/75.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/76.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/77.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/78.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/79.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/79.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
  { src: `${BASE}gallery/awards/82.jpg`, alt: "Подяка", tag: "Подяка", group: "awards" },
];

export const DONATE = {
  label: "Пожертва",
  href: "https://send.monobank.ua/jar/1iuuLBi5L", 
};

export const CURRENT_NEEDS = [
  { title: "Працьовиті руки" },
  { title: "Сітка-основа для маскування "},
  { title: "Рулонний спанбонд – легке неткане полотно (агроволокно)"},
  { title: "Мішковина для індивідуальних маскувальних костюмів – чугайстриків" },
  { title: "Шнури, нитки, ножиці" },
];

export const STATS = [
  {
    value: 97813,
    suffix: "+ м2",
    label: "маскувальних сіток",
    note: "За 12 років роботи",
  },
  {
    value: 437,
    suffix: "+",
    label: "чугайстриків",
    note: "Для маскування та укриття за 4 роки",
  },
  {
    value: 10000,
    suffix: "+",
    label: "килимків, шкарпеток, нашоломників, бафів, трусів, шапок",
    note: "Передано на фронт і в шпиталі",
  },
];
